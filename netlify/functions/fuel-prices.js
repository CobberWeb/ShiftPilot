...
function latOf(row){
  return Number(field(row,[
    "Site Latitude","Site_Latitude","Latitude","SiteLatitude","lat","Lat"
  ]));
}
function lonOf(row){
  return Number(field(row,[
    "Site Longitude","Site_Longitude","Longitude","SiteLongitude","lon","lng","Long"
  ]));
}
...
function latestPerStation(records){
  ...
  for(const r of records){
    const k=`${field(r,["Site ID","SiteId","Site_Id"])||field(r,["Site Name","Site_Name","SiteName"])}|${norm(field(r,["Fuel Type","Fuel_Type","FuelType"])||"")}`;
...
exports.handler=async function(event){
  try{
    ...
    let candidates=raw.filter(r=>fuelMatches(
      field(r,["Fuel Type","Fuel_Type","FuelType","fuel_type"])||"",
      fuelType
    ));
    ...
    if(useGps){
      ...
      candidates=candidates.filter(r=>{
        const s=norm(field(r,["Site Suburb","Site_Suburb","SiteSuburb"])||"");
    ...
    const rows=latest.map(r=>({
      siteId:field(r,["Site ID","SiteId","Site_Id"]),
      name:field(r,["Site Name","Site_Name","SiteName"])||"Service station",
      brand:field(r,["Site Brand","Site_Brand","SiteBrand"])||"",
      suburb:field(r,["Site Suburb","Site_Suburb","SiteSuburb"])||"",
      fuelType:field(r,["Fuel Type","Fuel_Type","FuelType"])||"",
      price:priceToDollars(field(r,["Price","price"])),
      timestamp:field(r,["Transaction DateUTC","TransactionDateutc","TransactionDateUTC","Transaction_DateUTC"])||null,
